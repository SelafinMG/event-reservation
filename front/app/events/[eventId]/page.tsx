...
export default function EventDetailPage({ params }: Props) {
  ...
  const sessionsByDay = event.sessions.reduce<Record<string, typeof event.sessions>>((acc, s) => {
      ...
      <div className="flex items-center gap-2 flex-wrap mb-7 animate-fade-up delay-3">
        ...
        {rooms.map(room => (
          <Link key={room.id} href={`/events/${event.id}?room=${room.id}`}
            ...
            style={{ background:"rgba(200,218,248,0.04)", border:"1px solid rgba(200,218,248,0.07)", color:"rgba(155,175,215,0.42)" }}>
            {room.name}
          </Link>
        ))}
      </div>
