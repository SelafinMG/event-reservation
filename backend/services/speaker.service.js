...
const fetchSpeakerById = async (speakerId) => {
  try {
    ...
    console.error('[fetchSpeakerById]', err.message);
    throw err;
  }
};
...
export const getAllSpeakersService = async () => {
  try {
    const { rows } = await pool.query(
      'SELECT id FROM speakers ORDER BY full_name'
    );
    return Promise.all(rows.map((row) => fetchSpeakerById(row.id)));
  } catch (err) {
    console.error('[getAllSpeakersService]', err.message);
    throw err;
  }
};
...
export const getSpeakerByIdService = async (speakerId) => {
  try {
    return await fetchSpeakerById(speakerId);
  } catch (err) {
    console.error('[getSpeakerByIdService]', err.message);
    throw err;
  }
};
...
export const createSpeakerService = async ({ fullName, photoUrl, bio, links }) => {
  try {
    ...
    console.error('[createSpeakerService]', err.message);
    throw err;
  }
};
...
export const updateSpeakerService = async (speakerId, { fullName, photoUrl, bio, links }) => {
  try {
    ...
    if (links !== undefined) {
      await pool.query(
        'DELETE FROM speaker_links WHERE speaker_id = $1',
        [speakerId]
      );
      for (const link of links) {
        await pool.query(
    ...
    if (err.code === '22P02') return null;
    console.error('[updateSpeakerService]', err.message);
    throw err;
  }
};
...
export const deleteSpeakerService = async (speakerId) => {
  try {
    ...
    console.error('[deleteSpeakerService]', err.message);
    throw err;
  }
};